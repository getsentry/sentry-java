...
class SentryApollo4HttpInterceptor
    ...
    constructor(
            ...
            if (!isIgnored()) {
                TracingUtils
                    .traceIfAllowed(
                        ...
                        request.headers
                            .filter {
                                it.name == BaggageHeader.BAGGAGE_HEADER
                            }.map { it.value },
                        span,
                    )?.let {
                        ...
                        it.baggageHeader?.let { baggageHeader ->
                            cleanedHeaders =
                                cleanedHeaders.filterNot { it.name == BaggageHeader.BAGGAGE_HEADER }.toMutableList().apply {
        ...
        private fun isIgnored(): Boolean = SpanUtils.isIgnored(scopes.getOptions().ignoredSpanOrigins, TRACE_ORIGIN)
        ...
        ): String? = headers.firstOrNull { it.name.equals(key, true) }?.value
