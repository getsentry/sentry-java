...
internal object DistributionInternal {
  ...
  private fun performUpdateCheck(context: Context): UpdateStatus {
    ...
    try {
      ...
      println("  Current build: ${parsedResponse.current?.let { "${it.buildVersion} (${it.buildNumber})" } ?: "null"}")
      println("  Update available: ${parsedResponse.update?.let { "${it.buildVersion} (${it.buildNumber})" } ?: "null"}")
