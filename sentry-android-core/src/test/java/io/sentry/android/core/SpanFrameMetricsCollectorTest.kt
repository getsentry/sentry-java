...
class SpanFrameMetricsCollectorTest {
  private class Fixture {
    ...
    fun getSut(enabled: Boolean = true): SpanFrameMetricsCollector {
      whenever(frameMetricsCollector.startCollection(any()))
  ...
  fun `when no frame data is collected the total count is interpolated`() {
    ...
    val span = createFakeSpan(TimeUnit.SECONDS.toNanos(1), TimeUnit.SECONDS.toNanos(2))
  ...
  fun `when no frame data is collected the total count is interpolated and frame delay is added`() {
    ...
    fixture.timeNanos = TimeUnit.SECONDS.toNanos(1)
    val span = createFakeSpan(TimeUnit.SECONDS.toNanos(1), TimeUnit.SECONDS.toNanos(3))
  ...
  fun `when frame data is only partially collected the total count is not interpolated in case the span didn't finish`() {
    ...
    fixture.timeNanos = TimeUnit.SECONDS.toNanos(1)
    val span =
      createFakeSpan(startTimeStampNanos = TimeUnit.SECONDS.toNanos(1), endTimeStampNanos = null)
  ...
  @Test
  fun `getFramesDelay returns -1 when disabled`() {
    val sut = fixture.getSut(enabled = false)

    val result = sut.getFramesDelay(0, TimeUnit.SECONDS.toNanos(1))
  ...
  fun `getFramesDelay returns -1 for invalid time range`() {
    ...
    // when start >= end
    val result = sut.getFramesDelay(2000, 1000)
    assertEquals(-1.0, result.delaySeconds)
    assertEquals(0, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay returns -1 for zero duration`() {
    val sut = fixture.getSut()

    val result = sut.getFramesDelay(1000, 1000)
    assertEquals(-1.0, result.delaySeconds)
    assertEquals(0, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay returns zero delay when no frames recorded`() {
    val sut = fixture.getSut()

    // given a span is running (so frames are being collected)
    val span = createFakeSpan(0, TimeUnit.SECONDS.toNanos(2))
    sut.onSpanStarted(span)

    // choreographer reports a recent timestamp, so no pending delay
    fixture.lastKnownChoreographerFrameTimeNanos = TimeUnit.SECONDS.toNanos(1)

    // but no slow/frozen frames are recorded
    val result = sut.getFramesDelay(0, TimeUnit.SECONDS.toNanos(1))
  ...
  @Test
  fun `getFramesDelay calculates delay from slow and frozen frames`() {
    val sut = fixture.getSut()
    ...
    // and one slow frame (32ms, 16ms delay) and one frozen frame (700ms, 684ms delay)
    sut.onFrameMetricCollected(100, 132, 32, 16, true, false, 60.0f)
    sut.onFrameMetricCollected(200, 900, 700, 684, true, true, 60.0f)

    // choreographer reports end of range, so no pending delay
    fixture.lastKnownChoreographerFrameTimeNanos = TimeUnit.SECONDS.toNanos(2)

    val result = sut.getFramesDelay(0, TimeUnit.SECONDS.toNanos(2))

    // delay should be (16 + 684) nanos = 700ns = 0.0000007 seconds
    assertEquals(700.0 / 1e9, result.delaySeconds, 0.0001)
    assertEquals(2, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay handles partial frame overlap at start`() {
    val sut = fixture.getSut()

    val ms = TimeUnit.MILLISECONDS

    // given a span is running
    val span = createFakeSpan(0, ms.toNanos(5000))
    sut.onSpanStarted(span)

    // and a frozen frame: 0ms-1000ms (1s duration, delay=984ms)
    sut.onFrameMetricCollected(
      ms.toNanos(0),
      ms.toNanos(1000),
      ms.toNanos(1000),
    ...
    // choreographer reports end of range
    fixture.lastKnownChoreographerFrameTimeNanos = ms.toNanos(2000)

    // query from 500ms to 2000ms — frame partially overlaps at start
    val result = sut.getFramesDelay(ms.toNanos(500), ms.toNanos(2000))

    // durationBeforeRange = 500ms, delayBeforeRange = max(0, 500ms - 16ms) = 484ms
    // delayWithinRange = min(984ms - 484ms, 1500ms) = 500ms
    // frameDuration = 1000ms - 500ms = 500ms, which is frozen (> 700ms? no, 500ms)
    // but it IS slow since 500ms > 16ms expected
    assertEquals(500.0 / 1e3, result.delaySeconds, 0.01)
    assertEquals(1, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay handles partial frame overlap at end`() {
    val sut = fixture.getSut()
    ...
    // and a frozen frame: 500ms-1500ms (1s duration, delay=984ms)
    sut.onFrameMetricCollected(
      ms.toNanos(500),
      ms.toNanos(1500),
      ms.toNanos(1000),
    ...
    // choreographer reports end of range
    fixture.lastKnownChoreographerFrameTimeNanos = ms.toNanos(800)

    // query from 0ms to 800ms — frame partially overlaps at end
    val result = sut.getFramesDelay(ms.toNanos(0), ms.toNanos(800))

    // frame starts at 500ms (>= 0), but end 1500ms > 800ms
    // partial overlap: frameStart=500ms, frameEnd=800ms, frameDuration=300ms
    // durationBeforeRange = 0, delayBeforeRange = 0
    // delayWithinRange = min(984ms, 800ms) = 800ms
    // 300ms > 16ms expected, so isSlow=true
    assertEquals(800.0 / 1e3, result.delaySeconds, 0.01)
    assertEquals(1, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay includes pending frame delay`() {
    val sut = fixture.getSut()

    // given a span is running
    val span = createFakeSpan(0, TimeUnit.SECONDS.toNanos(3))
    sut.onSpanStarted(span)

    // and no frames are drawn, but the choreographer reports a timestamp
    fixture.lastKnownChoreographerFrameTimeNanos = TimeUnit.SECONDS.toNanos(1)

    // query from 1s to 2s — 1s since last choreographer frame = pending frozen frame
    val result = sut.getFramesDelay(TimeUnit.SECONDS.toNanos(1), TimeUnit.SECONDS.toNanos(2))

    // pending duration = 2s - 1s = 1s, which is frozen (> 700ms)
    // pending delay = 1s - 16.67ms ≈ 983.33ms
    assertEquals(0.983333, result.delaySeconds, 0.01)
    assertEquals(1, result.framesContributingToDelayCount)
  }

  @Test
  fun `getFramesDelay does not require an active span for querying`() {
    val sut = fixture.getSut()

    val ms = TimeUnit.MILLISECONDS

    // given a span is running (to trigger frame collection)
    val span = createFakeSpan(0, ms.toNanos(2000))
    sut.onSpanStarted(span)

    // and frames are recorded
    sut.onFrameMetricCollected(
      ms.toNanos(10),
      ms.toNanos(42),
      ms.toNanos(32),
    ...
    // choreographer reports end of query range
    fixture.lastKnownChoreographerFrameTimeNanos = ms.toNanos(1000)

    // getFramesDelay can be called independently without finishing the span
    val result = sut.getFramesDelay(0, ms.toNanos(1000))
    assertEquals(700.0 / 1e3, result.delaySeconds, 0.001)
    assertEquals(2, result.framesContributingToDelayCount)

    // the span is still running and can still finish normally
    fixture.lastKnownChoreographerFrameTimeNanos = ms.toNanos(2000)
    sut.onSpanFinished(span)
    verify(span).setData("frames.slow", 1)
    verify(span).setData("frames.frozen", 1)
  }
}
