...
"perfetto_hprof_listener" prio=10 tid=7 Native (still starting up)
    ...
    verify(fixture.scopes)
      .captureEvent(
        check {
          ...
          val image =
            it.debugMeta?.images?.find { it.debugId == "741f3301-bbb0-b92c-58bd-c15282b8ec7b" }
          assertNotNull(image)
  ...
  fun `waits for ANR events to be flushed on disk`() {
    val integration =
      fixture.getSut(tmpDir, lastReportedAnrTimestamp = oldTimestamp, flushTimeoutMillis = 500L)
    fixture.addAppExitInfo(timestamp = newTimestamp)
  ...
  fun `when historical ANRs flag is disabled, does not report`() {
    val integration =
      fixture.getSut(tmpDir, lastReportedAnrTimestamp = oldTimestamp, reportHistoricalAnrs = false)
    fixture.addAppExitInfo(timestamp = newTimestamp - 2 * 60 * 1000)
    fixture.addAppExitInfo(timestamp = newTimestamp - 1 * 60 * 1000)
    fixture.addAppExitInfo(timestamp = newTimestamp)
  ...
  fun `attaches plain thread dump, if enabled`() {
    val integration =
      fixture.getSut(tmpDir, lastReportedAnrTimestamp = oldTimestamp, attachAnrThreadDump = true)
    fixture.addAppExitInfo(timestamp = newTimestamp)
  ...
  @Test
  fun `when ANR has only system frames, sets custom fingerprint`() {
