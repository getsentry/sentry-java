...
public class SpanFrameMetricsCollector
   ...
   * {@link io.sentry.time.MonotonicTicker} — {@code CLOCK_BOOTTIME} on Android — while frames are
   * on {@code CLOCK_MONOTONIC}, so crossing between them costs one offset read. The offset changes
  ...
  private static long toNanoTime(final @NotNull ISpan span, final @NotNull Timestamp timestamp) {
    ...
    if (anchor != null) {
      ...
      // The tick is on io.sentry.time.MonotonicTicker, which is CLOCK_BOOTTIME on Android, while
