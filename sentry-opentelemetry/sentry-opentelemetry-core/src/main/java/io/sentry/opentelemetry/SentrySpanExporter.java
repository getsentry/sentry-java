...
public final class SentrySpanExporter implements SpanExporter {
  ...
  public SentrySpanExporter(final @NotNull IHub hub) {
    this.hub = hub;
  }
  ...
  private boolean isSentryRequest(final @NotNull SpanData spanData) {
    ...
    if (DsnUtil.urlContainsDsnHost(hub.getOptions(), httpUrl)) {
      return true;
    }

    final @Nullable String fullUrl = spanData.getAttributes().get(SemanticAttributes.URL_FULL);
    return DsnUtil.urlContainsDsnHost(hub.getOptions(), fullUrl);
  }
  ...
  private @Nullable ITransaction createTransactionForOtelSpan(final @NotNull SpanData span) {
    ...
    final @Nullable IHub hubMaybe = spanStorage.getHub(span.getSpanContext());
    final @NotNull IHub hub = hubMaybe == null ? HubAdapter.getInstance() : hubMaybe;
    ...
    ITransaction sentryTransaction = hub.startTransaction(transactionContext, transactionOptions);
  ...
  public CompletableResultCode flush() {
    // TODO use correct hub?
    // TODO timeout?
    HubAdapter.getInstance().flush(10000);
    return CompletableResultCode.ofSuccess();
  }
