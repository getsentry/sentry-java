...
class ComputeNanoStartTimestampForChildTest {
  ...
  fun `returns the parent's start when no time has elapsed since it started`() {
    val span = anchoredSpan(WALL_CLOCK_MILLIS, FakeTicker())
  ...
  private fun anchoredSpan(wallClockMillis: Long, clock: FakeTicker): ISpan {
  ...
  private class FakeTicker : MonotonicTicker {
