...
class ResourcesDebugMetaLoaderTest {
  class Fixture {
    ...
    fun getSut(
      ...
      if (content != null) {
        val mockUrlMap = content.map { content -> Pair(mock<URL>(), content) }
        whenever(classLoader.getResources(fileName))
          .thenReturn(Collections.enumeration(mockUrlMap.map { it.first }))

        mockUrlMap.forEach {
          whenever(it.first.openStream())
            .thenReturn(it.second.byteInputStream(Charset.defaultCharset()))
        }
      } else {
        whenever(classLoader.getResources(fileName)).thenReturn(Collections.emptyEnumeration())
      }
  ...
  fun `reads multiple bundle IDs from multiple resource into properties and can be applied to options`() {
    ...
    assertEquals(
      options.bundleIds,
      setOf(
        "88ba82db-cd26-4c09-8b31-21461d286b68",
        "8d11a44a-facd-46c1-a49b-87d256227101",
        "13e16819-accf-48da-a82d-f6ec94af9948",
      ),
    )
