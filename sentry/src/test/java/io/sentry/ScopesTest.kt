...
class ScopesTest {
  ...
  @Test
  fun `startTransaction start the continuous profiler before creating SentryTracer in ProfileLifecycle TRACE`() {
    ...
    whenever(profiler.profilerId).thenReturn(SentryId.EMPTY_ID)

    val expectedSentryId = SentryId()

    doAnswer { whenever(profiler.profilerId).thenReturn(expectedSentryId) }
      .whenever(profiler)
      .startProfiler(eq(ProfileLifecycle.TRACE), any())

    val transaction = scopes.startTransaction("test", "test")

    val profilerId = transaction.getData("profiler_id") as? String
    val profilingContext = transaction.contexts.get("profile") as? ProfileContext
    assertNotNull(profilerId)
    assertTrue(SentryId(transaction.getData("profiler_id")!! as String) != SentryId.EMPTY_ID)
    assertEquals(expectedSentryId, SentryId(profilerId))
    assertEquals(ProfileContext(SentryId(profilerId)), profilingContext)
    verify(profiler).startProfiler(eq(ProfileLifecycle.TRACE), any())
  }
  ...
  fun `metric with non manual origin does have origin attribute`() {
    ...
    sut
      ...
      .count(
        "metric name",
        1.0,
        MeasurementUnit.Information.BYTE.apiName(),
  ...
  fun `creating count metric with value and unit works`() {
    ...
    sut.metrics().count("metric name", 1.0, MeasurementUnit.Information.BYTE.apiName())
    ...
    verify(mockClient)
      .captureMetric(
        check {
          assertEquals("metric name", it.name)
          assertEquals(1.0, it.value)
          assertEquals(MeasurementUnit.Information.BYTE.apiName(), it.unit)
          assertEquals("counter", it.type)
        },
        anyOrNull(),
        anyOrNull(),
  ...
  fun `creating count metric with unit works`() {
    ...
    sut.metrics().count("metric name", MeasurementUnit.Information.BYTE.apiName())
  ...
  fun `creating distribution metric with value and unit works`() {
    ...
    sut.metrics().distribution("metric name", 1.0, MeasurementUnit.Duration.MILLISECOND.apiName())
    ...
    verify(mockClient)
      .captureMetric(
        check {
          assertEquals("metric name", it.name)
          assertEquals(1.0, it.value)
          assertEquals(MeasurementUnit.Duration.MILLISECOND.apiName(), it.unit)
          assertEquals("distribution", it.type)
        },
        anyOrNull(),
        anyOrNull(),
  ...
  fun `creating gauge metric with value and unit works`() {
    ...
    sut.metrics().gauge("metric name", 128.0, MeasurementUnit.Information.BYTE.apiName())
