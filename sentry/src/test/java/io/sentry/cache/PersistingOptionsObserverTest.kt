...
class StoreOptionsValue<T>(
    private val store: PersistingOptionsObserver.(T) -> Unit,
) {
    operator fun invoke(
        value: T,
        observer: PersistingOptionsObserver,
    ) {
        observer.store(value)
    }
}

class DeleteOptionsValue(
    private val delete: PersistingOptionsObserver.() -> Unit,
) {
...
class ReadOptionsValue<T>(
    private val read: (options: SentryOptions) -> T,
) {
...
class PersistingOptionsObserverTest<T>(
    ...
    companion object {
        ...
        private fun proguardUuid(): Array<Any?> =
            arrayOf(
        ...
        private fun sdkVersion(): Array<Any?> =
            arrayOf(
        ...
        private fun replaysErrorSampleRate(): Array<Any?> =
            arrayOf(
                0.5,
