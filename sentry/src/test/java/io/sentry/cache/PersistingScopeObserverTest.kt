...
class StoreScopeValue<T>(
    private val store: PersistingScopeObserver.(T, Scope) -> Unit,
) {
    operator fun invoke(
        value: T,
        observer: PersistingScopeObserver,
        scope: Scope,
    ) {
...
class DeleteScopeValue(
    private val delete: PersistingScopeObserver.(Scope) -> Unit,
) {
    operator fun invoke(
        observer: PersistingScopeObserver,
        scope: Scope,
...
class DeletedEntityProvider<T>(
    private val provider: (Scope) -> T?,
) {
    operator fun invoke(scope: Scope): T? = provider(scope)
}
...
class PersistingScopeObserverTest<T>(
    ...
    private fun PersistingScopeObserver.read(): Any? =
        read(
            fixture.options,
            filename,
    ...
    companion object {
        ...
        private fun breadcrumbs(): Array<Any?> =
            arrayOf(
                listOf(
        ...
        private fun legacyBreadcrumbs(): Array<Any?> =
            arrayOf(
        ...
        private fun fingerprint(): Array<Any?> =
            arrayOf(
                listOf("finger", "print"),
        ...
        private fun contexts(): Array<Any?> =
            arrayOf(
                Contexts().apply {
                    setApp(
                        App().apply {
        ...
        private fun replayId(): Array<Any?> =
            arrayOf(
