...
public final class PersistingScopeObserver extends ScopeObserverAdapter {
  ...
  public static final String ATTACHMENTS_FILENAME = "attachments.json";
      ...
      () -> {
        ...
        try {
          try {
            ...
            // the tradeoff of losing breadcrumbs for ANRs that happened right before the app has
            // received an update where the new format was introduced
            file.delete();
          ...
          new ObjectQueue.Converter<Breadcrumb>() {
            ...
            public void toStream(Breadcrumb value, OutputStream sink) throws IOException {
              try (final Writer writer =
                     new BufferedWriter(new OutputStreamWriter(sink, StandardCharsets.UTF_8))) {
  ...
  public void addAttachment(final @NotNull Attachment attachment) {
    ...
    serializeToDisk(
      () -> {
        final List<PersistedAttachment> existing = readPersistedAttachments();
        existing.add(toPersistedAttachment(attachment));
        store(existing, ATTACHMENTS_FILENAME);
      });
  }
    ...
    serializeToDisk(
      () -> {
        if (attachments.isEmpty()) {
          delete(ATTACHMENTS_FILENAME);
        } else {
          final List<PersistedAttachment> persisted = new ArrayList<>();
          for (final Attachment attachment : attachments) {
            if (attachment.getPathname() != null) {
              persisted.add(toPersistedAttachment(attachment));
            }
          }
          if (persisted.isEmpty()) {
            delete(ATTACHMENTS_FILENAME);
          } else {
            store(persisted, ATTACHMENTS_FILENAME);
          }
        }
      });
  }
  ...
  private @NotNull PersistedAttachment toPersistedAttachment(final @NotNull Attachment attachment) {
    // callers ensure getPathname() is non-null before calling this method
    return new PersistedAttachment(
      Objects.requireNonNull(attachment.getPathname()),
  ...
  private @NotNull List<PersistedAttachment> readPersistedAttachments() {
    final List<PersistedAttachment> result =
      (List<PersistedAttachment>)
        CacheUtils.read(
          ...
          List.class,
          new PersistedAttachment.Deserializer());
  ...
  public <T> @Nullable T read(
    ...
    if (fileName.equals(ATTACHMENTS_FILENAME)) {
      return CacheUtils.read(
        options, SCOPE_CACHE, fileName, clazz, new PersistedAttachment.Deserializer());
    }
