...
public final class SentryWeakSpanStorage {
  ...
  private final @NotNull WeakConcurrentMap<SpanContext, IHub> hubs = new WeakConcurrentMap<>(true);
  private final @NotNull WeakConcurrentMap<SpanContext, Scopes> scopes =
      new WeakConcurrentMap<>(true);

  private SentryWeakSpanStorage() {}

  public @Nullable IHub getHub(final @NotNull SpanContext spanContext) {
    return hubs.get(spanContext);
  }

  public void storeHub(final @NotNull SpanContext spanContext, final @NotNull IHub hub) {
    hubs.put(spanContext, hub);
  }

  public @Nullable Scopes getScopes(final @NotNull SpanContext spanContext) {
    return scopes.get(spanContext);
  }

  public void storeScopes(final @NotNull SpanContext otelSpan, final @NotNull Scopes scopes) {
