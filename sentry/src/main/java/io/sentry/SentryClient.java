...
public class SentryClient {
    ...
    Event buildEvent(EventBuilder eventBuilder) {
        ...
        for (Map.Entry<String, String> tagEntry : tags.entrySet()) {
            Map<String, String> tags = workingEvent.getTags();
            String currentValue = tags.put(tagEntry.getKey(), tagEntry.getValue());
            // Default tags should not override event-specific tags:
            if (currentValue != null) {
                tags.put(tagEntry.getKey(), currentValue);
            }
        }

        for (Map.Entry<String, Object> extraEntry : extra.entrySet()) {
            Map<String, Object> extra = workingEvent.getExtra();
            Object currentValue = extra.put(extraEntry.getKey(), extraEntry.getValue());
            // Default extra should not override event-specific extra:
            if (currentValue != null) {
                extra.put(extraEntry.getKey(), currentValue);
            }
        }
