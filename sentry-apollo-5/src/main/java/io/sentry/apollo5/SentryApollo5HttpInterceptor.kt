...
private const val TRACE_ORIGIN = "auto.graphql.apollo5"

class SentryApollo5HttpInterceptor
@JvmOverloads
constructor(
  ...
  init {
    addIntegrationToSdkVersion("Apollo5")
    if (captureFailedRequests) {
      SentryIntegrationPackageStorage.getInstance().addIntegration("Apollo5ClientError")
    }
  }
  ...
  override suspend fun intercept(request: HttpRequest, chain: HttpInterceptorChain): HttpResponse {
    ...
    try {
      ...
      // client errors don't throw anymore in v5, but we should still be able to detect all of them
  ...
  private fun captureEvent(
    ...
    try {
      ...
      val mechanism = Mechanism().apply { type = "SentryApollo5Interceptor" }
      ...
      val exception = SentryApollo5ClientException(builder.toString())
  ...
  companion object {
    ...
    init {
      SentryIntegrationPackageStorage.getInstance()
        .addPackage("maven:io.sentry:sentry-apollo-5", BuildConfig.VERSION_NAME)
    }
  }
}
