...
import io.sentry.util.Apollo5PlatformTestManipulator
...
class SentryApollo5HttpInterceptorTestWithV5Implementation :
  SentryApollo5HttpInterceptorTest(ApolloCall<*>::execute)

abstract class SentryApollo5HttpInterceptorTest(
  ...
  class Fixture {
    ...
    private var httpInterceptor =
      SentryApollo5HttpInterceptor(scopes, captureFailedRequests = false)
  ...
  "data": {
    "launch": {
      ...
      if (beforeSpan != null) {
        httpInterceptor =
          SentryApollo5HttpInterceptor(scopes, beforeSpan, captureFailedRequests = false)
      }
  ...
  @Test
  fun `does not read response body if failed request target does not match`() = runBlocking {
    var bodyRead = false
    val body =
      object : ForwardingSource(Buffer().writeUtf8("{\"errors\":[]}")) {
        ...
        .buffer()
    val request = HttpRequest.Builder(HttpMethod.Post, "https://excluded.example/graphql").build()
    val response = HttpResponse.Builder(200).body(body).build()
    val chain =
  ...
  fun `does not add sentry-trace header when span origin is ignored`() {
    fixture.options.setIgnoredSpanOrigins(listOf("auto.graphql.apollo5"))
    executeQuery(isSpanActive = false)
  ...
  fun `sets SDKVersion Info`() {
    ...
    assert(fixture.scopes.options.sdkVersion!!.integrationSet.contains("Apollo5"))
  }

  @Test
  fun `attaches to root transaction on Android`() {
    Apollo5PlatformTestManipulator.pretendIsAndroid(true)
    executeQuery(fixture.getSut())
