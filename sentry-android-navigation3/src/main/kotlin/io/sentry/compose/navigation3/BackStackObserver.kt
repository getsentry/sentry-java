...
internal class BackStackObserver<T : Any>(
  ...
  internal companion object {
    ...
    private const val ON_BACK_STACK_CHANGED_TRACE_SECTION = "SentryNavEffect.onBackStackChanged"
  ...
  internal fun onBackStackChanged(backStack: List<T>) {
    ...
    guard("onBackStackChanged") {
      Trace.beginSection(ON_BACK_STACK_CHANGED_TRACE_SECTION)
      try {
        scopes.configureScope { scope ->
          ...
          if (previousTop === currentTop) {
            // Return based on identity rather than structural equality to match what
            // SentryNavEffect
            // does via BackStackKey.
            return@configureScope
          }
  ...
  private fun handleNewTop(
    ...
    if (areNavigationTransactionsEnabled) {
      ...
      // Keep error grouping identical whether nav transactions are enabled or not.
...
private class NavTransactionManager(
  ...
  fun start(
    ...
    // Track the transaction immediately so a later failure doesn't orphan it.
