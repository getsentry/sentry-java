...
internal fun <T : Any> rememberSentryNavStateHolder(
  backStack: SentryNavBackStack<T>,
  options: SentryNavOptions,
  scopes: IScopes,
): SentryNavStateHolder<T> {
  val backStackSnapshot = backStack.entries.toList()
  val currentNameExtractor = rememberUpdatedState(backStack.nameExtractor)
  val currentArgumentsExtractor = rememberUpdatedState(backStack.argumentsExtractor)
  ...
  val holder =
    remember(
      ...
      SentryNavStateHolder(
        ...
        nameExtractor = backStack.nameExtractor,
        argumentsExtractor = backStack.argumentsExtractor,
      )
    }

  DisposableEffect(holder) { onDispose { holder.cleanup() } }

  DisposableEffect(holder, backStackSnapshot) {
    holder.nameExtractor = currentNameExtractor.value
    holder.argumentsExtractor = currentArgumentsExtractor.value
    holder.onBackStackChanged(backStackSnapshot)
