...
class SentryNavEntryDecoratorTest {
  ...
  private fun <T : Any> TestSentryNavEntryDecorator(
    ...
    NavDisplay(
      backStack = backStack,
      entryDecorators = listOf(rememberSaveableStateHolderNavEntryDecorator(), sentryDecorator),
  ...
  fun `initial backstack fires breadcrumb and transaction`() {
    ...
    composeRule.setContent {
      TestSentryNavEntryDecorator(backStack = backStack, scopes = fixture.scopes)
    }
  ...
  fun `changing captureBackStack to false clears navigation context`() {
    ...
    composeRule.setContent {
      TestSentryNavEntryDecorator(
        backStack = backStack,
        scopes = fixture.scopes,
        options = SentryNavOptions().apply { this.captureBackStack = captureBackStack.value },
      )
    }
    composeRule.waitForIdle()
  ...
  fun `immutable backstack replacements preserve navigation history`() {
    ...
    composeRule.setContent {
      TestSentryNavEntryDecorator(backStack = backStack.value, scopes = fixture.scopes)
    }
    composeRule.waitForIdle()
