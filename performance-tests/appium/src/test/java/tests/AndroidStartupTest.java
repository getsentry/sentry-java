...
public class AndroidStartupTest {
  ...
  private static CommandEvent getCommand(List<CommandEvent> list, String name, int index)
      throws Exception {
    Optional<CommandEvent> result =
        list.stream().filter((cmd) -> cmd.getName().equals(name)).skip(index).findFirst();
