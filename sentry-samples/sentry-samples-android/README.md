...
## Nav Sample Transaction Tips

When inspecting route-level transactions from `Nav2Activity` or `Nav3Activity`, note that the sample app also enables automatic Activity tracing by default.
That can produce overlapping `ui.load` transactions for the Activity itself, which may make it harder to isolate the navigation transactions generated by the Nav2 or Nav3 integrations.

If you want the route transactions to be the primary trace surface while testing those sample screens, temporarily disable automatic Activity tracing in
