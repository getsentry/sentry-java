...
@Composable
fun Landing(navigateGithub: () -> Unit, navigateGithubWithArgs: () -> Unit) {
  ...
  SentryTraced(tag = "buttons_page") {
    Column(
      ...
      SentryTraced(tag = "button_nav_github") {
        Button(onClick = { navigateGithub() }, modifier = Modifier.padding(top = 32.dp)) {
          Text("Navigate to Github")
        }
      }
      SentryTraced(tag = "button_nav_github_args") {
        Button(onClick = { navigateGithubWithArgs() }, modifier = Modifier.padding(top = 32.dp)) {
...
@Composable
fun Github(user: String = "getsentry", perPage: Int = 30) {
  ...
  SentryTraced("github-$user") {
    Column(
      ...
      TextField(value = user, onValueChange = { newText -> user = newText })
      ...
      Button(
        ...
        modifier = Modifier.testTag("button_list_repos_async").padding(top = 32.dp),
      ) {
...
fun SampleNavigation(navController: NavHostController) {
  SentryTraced(tag = "navhost") {
    NavHost(navController = navController, startDestination = Destination.Landing.route) {
      ...
      composable(Destination.Github.route) { Github() }
      composable(
