...
private fun Nav2ComposeProductDetailRoute(
  ...
  Nav2ComposeActionRoute(
    ...
    footerButtons = listOf("Emit a span" to { emitSampleNavigationSpan(routeSpec.routeName) }),
  )
}
...
private fun Nav2ComposeActionRoute(
  ...
  footerButtons: List<Pair<String, () -> Unit>> = emptyList(),
) {
  Nav2ComposeRouteScaffold(
    routeSpec = routeSpec,
    cardContent = {
    ...
    footerContent = {
      footerButtons.forEach { (label, onClick) -> Nav2ComposeRouteButton(label, onClick) }
    },
  )
}
