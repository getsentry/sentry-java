...
import io.sentry.asyncprofiler.vendor.asyncprofiler.jfr.ClassRef;
import io.sentry.asyncprofiler.vendor.asyncprofiler.jfr.Dictionary;
import io.sentry.asyncprofiler.vendor.asyncprofiler.jfr.JfrReader;
import io.sentry.asyncprofiler.vendor.asyncprofiler.jfr.MethodRef;
...
import io.sentry.asyncprofiler.vendor.asyncprofiler.jfr.event.MallocLeakAggregator;
...
public abstract class JfrConverter extends Classifier {
  ...
  protected final @NotNull EventCollector collector;
  protected @NotNull Dictionary<String> methodNames;

  public JfrConverter(@NotNull JfrReader jfr, @NotNull Arguments args) {
    this.jfr = jfr;
    this.args = args;
  ...
  public StackTraceElement getStackTraceElement(long methodId, byte methodType, int location) {
    ...
    String classStr =
        className == null || className.length == 0 || isNativeFrame(methodType)
            ? ""
  ...
  public String getPlainThreadName(int tid) {
    String threadName = jfr.threads.get(tid);
  ...
  protected boolean isNativeFrame(byte methodType) {
    ...
    return (methodType == TYPE_NATIVE
            && jfr.getEnumValue("jdk.types.FrameType", TYPE_KERNEL) != null)
        || methodType == TYPE_CPP
