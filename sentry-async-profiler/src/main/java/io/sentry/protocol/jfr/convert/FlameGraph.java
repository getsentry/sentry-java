...
import static io.sentry.protocol.jfr.convert.ResourceProcessor.*;
...
public final class FlameGraph implements Comparator<Frame> {
  ...
  public FlameGraph(@NotNull Arguments args) {
    this.args = args;
    this.order = new int[0]; // Initialize with empty array
  }
  ...
  private void printFrame(PrintStream out, Frame frame, int level, long x) {
    ...
    boolean hasExtraTypes =
        (frame.inlined | frame.c1 | frame.interpreted) != 0
    ...
    if (frame.total != lastTotal || hasExtraTypes) {
      ...
      if (hasExtraTypes) {
        sb.append(',')
            .append(frame.inlined)
            .append(',')
            .append(frame.c1)
            .append(',')
            .append(frame.interpreted);
      }
    }
    sb.append(")\n");
