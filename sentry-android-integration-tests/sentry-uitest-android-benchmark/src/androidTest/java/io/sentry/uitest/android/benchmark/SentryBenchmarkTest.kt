...
class SentryBenchmarkTest : BaseBenchmarkTest() {
  ...
  fun benchmarkSameOperation() {
    // We compare two operation that are the same. We expect the increases to be negligible, as the
    // results
    ...
    // The fps decrease comparison is skipped, due to approximation: 59.51 and 59.49 fps are
  ...
  private fun getOperation(
    ...
    val benchmarkScenario =
      launchActivity<BenchmarkActivity>(
        activityOptions =
          Bundle().apply { putBoolean(BenchmarkActivity.EXTRA_SUSTAINED_PERFORMANCE_MODE, true) }
      )
    // Starts a transaction (it can be null, but we still runOnMainSync to make operations as
    // similar as possible)
    runner.runOnMainSync { transaction = transactionBuilder() }
    // Just swipe the list some times: this is the benchmarked operation
    swipeList(2)
    // We finish the transaction. We do it on main thread, so there's no need to perform other
    // operations after it
