...
class Nav3PerformanceBenchmark {
  ...
  fun interleavedAbComparison() {
    benchmarkRule.measureRepeated(
      ...
      metrics =
        listOf(
          Nav3CostAttributionMetric(NORMAL_AB_THRESHOLDS, restrictToAbPhases = true),
  ...
  private fun measurePerformanceRun(
    ...
    val metrics = buildList {
      ...
      if (includeCostAttribution) {
        add(
          Nav3CostAttributionMetric(
            regressionThresholds(preset, run),
            allowNoSlices = run == "UNRELATED_RECOMPOSITIONS",
          )
        )
      }
    }
  ...
  private class Nav3CostAttributionMetric(
    private val thresholds: Nav3RegressionThresholds,
    private val restrictToAbPhases: Boolean = false,
    private val allowNoSlices: Boolean = false,
  ) : TraceMetric() {
    override fun getMeasurements(
      ...
      val abPhaseCte =
        if (restrictToAbPhases) {
          ...
          ab_phases AS (
            ...
            WHERE slice.name IN ('$AB_DISABLED_SECTION', '$AB_ENABLED_SECTION')
              AND slice.dur > 0
          ),
          """
            .trimIndent()
      ...
      val abPhaseFilter =
        if (restrictToAbPhases) {
          ...
          AND EXISTS (
            SELECT 1
            FROM ab_phases AS phase
            WHERE phase.process_name = '$packageName'
              AND slice.ts >= phase.ts
              AND slice.ts + slice.dur <= phase.ts + phase.dur
          )
          """
            .trimIndent()
        } else {
      ...
      val ctesPrefix = if (abPhaseCte.isEmpty()) "" else "$abPhaseCte\n"
      val operations =
        traceSession
          .query(
            """
            WITH
            ${ctesPrefix}target_effects AS (
      ...
      if (operations.isEmpty()) {
        ...
        return listOf(
          Metric.Measurement("nav3IntegrationOperationCount", 0.0),
          Metric.Measurement("nav3CapturedEntryResolutionCount", 0.0),
          Metric.Measurement("nav3IntegrationTotalMs", 0.0),
          Metric.Measurement("nav3ExtractorTotalMs", 0.0),
          Metric.Measurement("nav3NonExtractorTotalMs", 0.0),
        )
      }
