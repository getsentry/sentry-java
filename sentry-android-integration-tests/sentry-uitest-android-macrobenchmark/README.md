...
`Nav3CostAttributionMetric` joins each `SentryNavEffect.onBackStackChanged` trace section to its name and
...
example, a 20-operation Normal run should report 20 integration operations and 840 captured-entry
resolutions. The count reflects every name extraction nested inside the integration work, including
back stack capture, breadcrumb naming, and transaction snapshotting. An unrelated recomposition run
should report 20 lightweight wrapper calls and zero captured-entry resolutions.

The benchmark now fails if the expected trace sections are missing or incomplete. A renamed or
missing `SentryNavEffect.onBackStackChanged`, `Nav3Stress.navigationToComposition`, or
